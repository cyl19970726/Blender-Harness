...
- 不管路径：资产该落哪 → `docs/ASSET_LAYOUT.md`（全量渲染 `.artifacts/blender-harness/<candidate>/`；boards/audit/review `docs/research/ar-magnet/<slug>/<candidate>/`；SBS 视频走 CloudBase）。
...
- **资产去处（渲染/boards/marker/GLB/音频/SBS 各落哪）**：`docs/ASSET_LAYOUT.md`（登记才算存在 → `docs/reference/ar-assets/manifest.json`）。
