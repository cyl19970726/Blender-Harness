...
首个纵切包含：路线 revision DAG、探针双状态、不可变证据、独立 review、知识 proposal/adjudication、真实 Blender Quicklook、Hunyuan capability registry 和可恢复作业。Quicklook v1 为保证缓存身份正确，暂只接受自包含 GLB；默认只允许单个可见几何，多对象场景必须显式选择 `whole_scene`。预算当前在 route/probe 中记录，runner timeout 会执行；跨 provider 的统一成本账本尚未实现，不能声称已经自动控费。
